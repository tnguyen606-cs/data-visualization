...
function createCountryBubble(svg, goalsAgainst, goalsFor, gamesPlayed, country) {
    ...
    // If United States, Germany or Brazil add a label below the bubble
    var name = country['Country'];
    if(name == 'United States' || name == 'Germany' || name == 'Brazil'){
        // Add a text new text element
        var text = svg.append('text');
        // Class the text element to use CSS for font-styling
        text.attr('class', 'country-name');
        // X-position inline with the circle
        text.attr('x', goalsAgainst);
        // Shift the text position down below the circle
        text.attr('y', goalsFor + gamesPlayed);
        // Using dy and setting it to an em value (em is a measurement relative to font size)
        // is a great way to move text by text-line heights
        text.attr('dy', '1.1em');

        // Set text to be country name
        text.text(name);
    }
